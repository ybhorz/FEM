...
%[text] - $P\_h = \\{ q \\in L^2 (\\Omega) : q|\_K \\in P\_1 (K), \\forall K \\in T\_h; \\ \[q\]\_e = 0, \\forall e \\in F^{pr, o}\_h \\}$ \
...
%[text] - Nodal DoF: $(v \\cdot n) \\cdot n |\_c$ and $(v \\cdot n) \\cdot t |\_c$ at edge vertex \
%[text] Matrix $SDG\_1$ element space
%[text] - $\\Sigma\_h = \\{ \\tau \\in L^2 (\\Omega, R^{2 \\times 2}) : \\tau|\_K \\in P\_1 (K)^{2 \\times 2}, \\forall K \\in T\_h; \\ \[\\tau \\cdot n\]\_e = 0, \\forall e \\in F^{pr, o}\_h; \\ \[(\\tau \\cdot n) \\cdot t\]\_e = 0, \\forall e \\in F^{dl}\_h \\}$ \
% SDG_1M_FE = FE("D2T", FE.repFS("[1,x,y]", [2, 2]), ...
%     [NdDoF("D2", D2TElem, 1, [0, 1], d0_s, "coef", [UNV, UNV], "EntIdx", 1, "form", @(coef, fcn) dot(fcn * coef(1), coef(2))), ...
%     NdDoF("D2", D2TElem, 1, [0, 1], d0_s, "coef", [UNV, UNV], "EntIdx", [2, 3], "form", @(coef, fcn) dot(fcn * coef(1), coef(2)), "share", false), ...
%     NdDoF("D2", D2TElem, 1, [0, 1], d0_s, "coef", [UNV, UTV], "form", @(coef, fcn) dot(fcn * coef(1), coef(2)))]);
load("SDG_1M_FE.mat");
...
%[text] - Nodal DoF: $q|\_c$ at midpoint of one edge \
%[text] Scalar $SDG\_0$ element space
%[text] - $P\_h = \\{ q \\in L^2 (\\Omega) : q|\_K \\in P\_0 (K), \\forall K \\in T\_h; \\ \[q\]\_e = 0, \\forall e \\in F^{pr, o}\_h \\}$ \
...
%[text] - Nodal DoF: $v \\cdot n |\_c$ at midpoint of two edge \
...
Bsv = [DLF(msh, 2, Fcn.cst(v), d0_s, grad_v, "iTrl", is, "iTst", iv, "GInt", GInt("D2T", ord_Sh + ord_Uh - 1)), ...
    DLF.interface(msh, 1, [-UNV * v, UTV, UTV], d0_s, d0_v, "EntIdx", DlEdge, "iTrl", is, "iTst", iv, "tstOpr", "jump", "form", @(coef, trl, tst) dot(trl * coef(1), coef(2)) .* dot(tst, coef(3)), "GInt", GInt("D2L", ord_Sh + ord_Uh)), ...
    DLF.interface(msh, 1, -UNV * v, d0_s, d0_v, "EntIdx", PrEdge, "iTrl", is, "iTst", iv, "tstOpr", "jump", "form", @(coef, trl, tst) dot(trl * coef, tst), "GInt", GInt("D2L", ord_Sh + ord_Uh))];
